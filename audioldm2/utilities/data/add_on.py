...
def get_vits_phoneme_ids(config, dl_output, metadata):
    ...
    assert "phonemes" in metadata.keys(), "You must provide vits phonemes on using addon get_vits_phoneme_ids"
...
def calculate_mel_spec_relative_bandwidth_as_extra_channel(config, dl_output, metadata):
    ...
    return {"mel_spec_bandwidth_cond_extra_channel": bandwidth_condition, "freq_energy_percentile": torch.LongTensor([lower_idx, higher_idx])}
...
def waveform_rs_48k(config, dl_output, metadata):
    ...
    if(sampling_rate != 48000):
        waveform_48k = torchaudio.functional.resample(waveform, orig_freq=sampling_rate, new_freq=48000)
    else:
...
def extract_vits_phoneme_and_flant5_text(config, dl_output, metadata):
    assert "phoneme" not in metadata.keys(), "The metadata of speech you use seems belong to fastspeech. Please check dataset_root.json"
...
def extract_fs2_phoneme_g2p_en_feature(config, dl_output, metadata):
    ...
    phonemes_lookup_dict = {'K': 0, 'IH2': 1, 'NG': 2, 'OW2': 3, 'AH2': 4, 'F': 5, 'AE0': 6, 'IY0': 7, 'SH': 8, 'G': 9, 'W': 10, 'UW1': 11, 'AO2': 12, 'AW2': 13, 'UW0': 14, 'EY2': 15, 'UW2': 16, 'AE2': 17, 'IH0': 18, 'P': 19, 'D': 20, 'ER1': 21, 'AA1': 22, 'EH0': 23, 'UH1': 24, 'N': 25, 'V': 26, 'AY1': 27, 'EY1': 28, 'UH2': 29, 'EH1': 30, 'L': 31, 'AA2': 32, 'R': 33, 'OY1': 34, 'Y': 35, 'ER2': 36, 'S': 37, 'AE1': 38, 'AH1': 39, 'JH': 40, 'ER0': 41, 'EH2': 42, 'IY2': 43, 'OY2': 44, 'AW1': 45, 'IH1': 46, 'IY1': 47, 'OW0': 48, 'AO0': 49, 'AY0': 50, 'EY0': 51, 'AY2': 52, 'UH0': 53, 'M': 54, 'TH': 55, 'T': 56, 'OY0': 57, 'AW0': 58, 'DH': 59, 'Z': 60, 'spn': 61, 'AH0': 62, 'sp': 63, 'AO1': 64, 'OW1': 65, 'ZH': 66, 'B': 67, 'AA0': 68, 'CH': 69, 'HH': 70}
    pad_token_id = len(phonemes_lookup_dict.keys())

    assert "phoneme" in metadata.keys(), "The dataloader add-on extract_phoneme_g2p_en_feature will output phoneme id, which is not specified in your dataset"

    phonemes = [phonemes_lookup_dict[x] for x in metadata["phoneme"] if (x in phonemes_lookup_dict.keys())]
    
    if((len(phonemes) / PAD_LENGTH) > 5):
        print("Warning: Phonemes length is too long and is truncated too much! %s" % metadata)
...
def extract_phoneme_g2p_en_feature(config, dl_output, metadata):
    ...
    phonemes_lookup_dict = {' ': 0, 'AA': 1, 'AE': 2, 'AH': 3, 'AO': 4, 'AW': 5, 'AY': 6, 'B': 7, 'CH': 8, 'D': 9, 'DH': 10, 'EH': 11, 'ER': 12, 'EY': 13, 'F': 14, 'G': 15, 'HH': 16, 'IH': 17, 'IY': 18, 'JH': 19, 'K': 20, 'L': 21, 'M': 22, 'N': 23, 'NG': 24, 'OW': 25, 'OY': 26, 'P': 27, 'R': 28, 'S': 29, 'SH': 30, 'T': 31, 'TH': 32, 'UH': 33, 'UW': 34, 'V': 35, 'W': 36, 'Y': 37, 'Z': 38, 'ZH': 39}
    pad_token_id = len(phonemes_lookup_dict.keys())
...
def extract_kaldi_fbank_feature_32k(config, dl_output, metadata):
    ...
    if(sampling_rate != 32000):
        waveform_32k = torchaudio.functional.resample(waveform, orig_freq=sampling_rate, new_freq=32000)
    else:
        waveform_32k = waveform

    waveform_32k = waveform_32k - waveform_32k.mean()
    fbank = torchaudio.compliance.kaldi.fbank(waveform_32k, htk_compat=True, sample_frequency=32000, use_energy=False,
                                                  window_type='hanning', num_mel_bins=128, dither=0.0, frame_shift=10)
...
def extract_drum_beat(config, dl_output, metadata):
    def visualization(conditional_signal, mel_spectrogram, filename):
        ...
        sf.write(os.path.basename(dl_output["fname"]), np.array(dl_output["waveform"])[0], dl_output["sampling_rate"])
    ...
    beat = [x-random_start_sample for x in metadata["beat"] if (x-random_start_sample >= 0 and x-random_start_sample <= original_segment_length_before_resample)] 
    ...
    latent_shape = (config["model"]["params"]["latent_t_size"], config["model"]["params"]["latent_f_size"])
    ...
    for each in beat:
        beat_index = int((each/original_segment_length_before_resample) * latent_shape[0])
    ...
    return {
        "cond_beat_downbeat": conditional_signal
    }
