...
from transformers import SpeechT5Processor, AutoTokenizer, GPT2Model, GPT2Tokenizer, PretrainedConfig, T5Config

from audioldm2.audiomae_gen.sequence_input import Sequence2AudioMAE
import numpy as np
from audioldm2.audiomae_gen.utils import Prenet
...
class PhonemeEncoder(nn.Module):
    def __init__(self, vocabs_size=41, pad_length=250, pad_token_id=None):
        ...
        self.text_encoder = TextEncoder(n_vocab = vocabs_size, 
                                        out_channels=192, 
                                        hidden_channels=192, 
                                        filter_channels=768,
                                        n_heads=2,
        ...
        self.learnable_positional_embedding = torch.nn.Parameter(torch.zeros((1, 192, self.PAD_LENGTH))) # [batchsize, seqlen, padlen]
        self.learnable_positional_embedding.requires_grad=True

    # Required
    def get_unconditional_condition(self, batchsize):
        unconditional_tokens = self.pad_token_sequence.expand(batchsize, self.PAD_LENGTH)
    ...
    def forward(self, phoneme_idx):
        ...
        return [text_emb.permute(0,2,1), text_emb_mask.squeeze(1)] # [2, 250, 192], [2, 250]
...
class FlanT5HiddenState(nn.Module):
    ...
    import ipdb;ipdb.set_trace()
    """
    def __init__(self, text_encoder_name="google/flan-t5-large", freeze_text_encoder=True):
        super().__init__()
    ...
    def encode_text(self, prompt):
        ...
        batch = self.tokenizer(
            prompt, max_length=128, # self.tokenizer.model_max_length 
...
class SequenceGenAudioMAECond(Sequence2AudioMAE):
    def __init__(self, 
                        ...
                        use_gt_mae_prob=None): # The prob of using AudioMAE GT
        if(use_warmup):
            use_warmup=False

        super().__init__(base_learning_rate=base_learning_rate, 
    ...
    def get_unconditional_condition(self, batchsize):
        ...
        return_dict["crossattn_audiomae_generated"] = [return_dict["crossattn_audiomae_pooled"][0], torch.ones_like(return_dict["crossattn_audiomae_pooled"][1]).float()] 
        return return_dict
    ...
    def forward(self, batch):
        ...
        input_embeds_mask = torch.ones((input_embeds.size(0), input_embeds.size(1))).to(input_embeds.device).float()
        ret_dict["crossattn_audiomae_generated"] = [input_embeds, input_embeds_mask] # Input sequence and mask
...
class AudioMAEConditionCTPoolRandTFSeparated(nn.Module):
    """
        ...
        exit(0)
    """
    def __init__(self, time_pooling_factors=[1,2,4,8], freq_pooling_factors=[1,2,4,8], eval_time_pooling=None, eval_freq_pooling=None, mask_ratio=0.0, regularization=False, no_audiomae_mask=True, no_audiomae_average=False):
        super().__init__()
        self.device = None
    ...
    def get_unconditional_condition(self, batchsize):
        ...
        return [torch.zeros((batchsize, token_num, 768)).to(device).float(), torch.ones((batchsize, token_num)).to(device).float()]
    ...
    def pool(self, representation, time_pool=None, freq_pool = None):
        ...
        if(self.training):
            if(time_pool is None and freq_pool is None):
                time_pool = min(64, self.time_pooling_factors[np.random.choice(list(range(len(self.time_pooling_factors))))])
                freq_pool = min(8, self.freq_pooling_factors[np.random.choice(list(range(len(self.freq_pooling_factors))))])
        ...
        pooled = (self.avgpooling(representation) + self.maxpooling(representation)) / 2 # [bs, embedding_dim, time_token_num, freq_token_num]
    ...
    def forward(self, batch, time_pool=None, freq_pool=None):
        ...
        with torch.no_grad():
            representation = self.audiomae(batch, mask_ratio=self.mask_ratio, no_mask=self.no_audiomae_mask, no_average=self.no_audiomae_average)
