...
class Sequence2AudioMAE(nn.Module):
    ...
    def cfg_uncond(self, batch_size):
        ...
        assert "crossattn_audiomae_pooled" in unconditional_conditioning.keys(), "The module is not initialized with AudioMAE"
        unconditional_conditioning["crossattn_clap_to_audiomae_feature"] = unconditional_conditioning["crossattn_audiomae_pooled"]
    ...
    def truncate_sequence_and_mask(self, sequence, mask, max_len = 512):
        if(sequence.size(1) > max_len):
            print("The input sequence length to GPT-2 model is too long:", sequence.size(1))
    ...
    def get_input_sequence_and_mask(self, cond_dict):
        ...
        for _id, sequence_key in enumerate(self.sequence_input_key):
            ...
            if(isinstance(cond_embed, list)):
                assert len(cond_embed) == 2, "The crossattn returned list should have length 2, including embed and attn_mask"
                ...
                if(input_embeds is None and input_embeds_attn_mask is None):
                    input_embeds, input_embeds_attn_mask = item_input_embeds, item_attn_mask
                else:
                    input_embeds = torch.cat([input_embeds, item_input_embeds], dim=1) # The 1-st dimension is time steps
                    input_embeds_attn_mask = torch.cat([input_embeds_attn_mask, item_attn_mask], dim=1) # The 1-st dimension is time steps
            else:
                assert isinstance(cond_embed, torch.Tensor)
                cond_embed = self.input_sequence_embed_linear[_id](cond_embed)
                attn_mask = torch.ones((cond_embed.size(0), cond_embed.size(1))).to(cond_embed.device)
                ...
                if(input_embeds is None and input_embeds_attn_mask is None):
                    ...
                    input_embeds, input_embeds_attn_mask = torch.cat([input_embeds, item_input_embeds], dim=1), torch.cat([input_embeds_attn_mask, item_attn_mask], dim=1)

        assert input_embeds is not None and input_embeds_attn_mask is not None
        
        input_embeds, input_embeds_attn_mask = self.truncate_sequence_and_mask(input_embeds, input_embeds_attn_mask, int(1024-self.mae_token_num))
        cond_sequence_end_time_idx = input_embeds.size(1) # The index that we start to collect the output embeds
    ...
    def mask_target_sequence(self, target_embeds, target_embeds_attn_mask):
        ...
        if(self.target_tokens_mask_ratio > 1e-4):
            ...
            time_seq_mask = (torch.rand((batchsize, time_seq_len)) > mask_ratio).to(target_embeds.device)
    ...
    def generate_partial(self, batch, cond_dict = None, no_grad=False):
        ...
        target_embeds, target_embeds_attn_mask =  cond_dict['crossattn_audiomae_pooled'][0], cond_dict['crossattn_audiomae_pooled'][1]
        
        target_time_steps = target_embeds.size(1)

        input_embeds, input_embeds_attn_mask, cond_sequence_end_time_idx = self.get_input_sequence_and_mask(cond_dict)
        
        model_input = torch.cat([input_embeds, target_embeds[:,:target_time_steps // 4,:]],dim=1)
        model_input_mask = torch.cat([input_embeds_attn_mask, target_embeds_attn_mask[:,:target_time_steps // 4]],dim=1)
        
        steps = self.mae_token_num
        
        for _ in range(3 * steps // 4):
            output = self.model(inputs_embeds=model_input, attention_mask=model_input_mask)["last_hidden_state"]
            # Update the model input
            model_input = torch.cat([model_input, output[:, -1:, :]], dim=1)
            # Update the attention mask
            attention_mask_new_step = torch.ones((model_input_mask.size(0), 1)).to(model_input.device)
            model_input_mask = torch.cat([model_input_mask, attention_mask_new_step], dim=1)
